...
B_swaps(1)=interpB(dates,discounts,datesSwaps(1));

dates_aux=[dates(1);datesSwaps];

% Compute all swaps discounts, yearfrac 30/360 EU
for ii=2:l
    B_swaps(ii)=(1-mid(ii)*sum(yearfrac(dates_aux(1:ii-1),dates_aux(2:ii),6).*B_swaps(1:ii-1)))/(1+yearfrac(datesSwaps(ii-1),datesSwaps(ii),6)*mid(ii));
end
