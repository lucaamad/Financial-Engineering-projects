...
IG_h_curve =  hcurve([IG_Bond_dirty_price_1y;IG_Bond_dirty_price_2y],[IG_cf_schedule_1y;IG_cf_schedule_2y],ZC_curve,R);
HY_h_curve =  hcurve([HY_Bond_dirty_price_1y;HY_Bond_dirty_price_2y],[HY_cf_schedule_1y;HY_cf_schedule_2y],ZC_curve,R);
...
IG_z_1y = zspread(IG_Bond_dirty_price_1y,IG_cf_schedule_1y,ZC_curve);
IG_z_2y = zspread(IG_Bond_dirty_price_2y,IG_cf_schedule_2y,ZC_curve);
HY_z_1y = zspread(HY_Bond_dirty_price_1y,HY_cf_schedule_1y,ZC_curve);
HY_z_2y = zspread(HY_Bond_dirty_price_2y,HY_cf_schedule_2y,ZC_curve);
...
% Report default probabilities
pig=1-exp(-IG_h_curve(1,2));
phy=1-exp(-HY_h_curve(1,2));
pig2=1-exp(-IG_h_curve(1,2)-IG_h_curve(2,2));
phy2=1-exp(-HY_h_curve(1,2)-HY_h_curve(2,2));

% By Chapman-Kolmogorov (and by invariance in time) we have that the square
% of the rating transition matrix is equal to the transition matrix of two
% years so we impose the equality and we find the values with fzero
% (knowing the sum of each row is equal to one)
q1=@(x) 2*pig-x*pig-pig^2+x*phy-pig2;
q2=@(x) pig-x*pig-pig*phy+x*phy+phy-phy2;
x=fzero(q1,0);
y=fzero(q2,0);
Q=[1-x-pig, x, pig; 1-y-phy,y,phy;0,0,1];
