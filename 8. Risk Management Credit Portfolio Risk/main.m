...
%N = 100000;
%N = N * 1000;       % Uncomment for convergence test (10,000,000 scenarios)
N = 1e6;

%% Q1: CreditMetrics' FV (let's start with all bonds rated IG)

%  Compute present Value in a year's time
FV = FV_risky_bond(IG_cf_schedule_2y, Q, ZC_curve, Recovery); 
E_FV = Q(1,:)*FV;   

disp("––– Part I Q1: Present Value in a year's time of the IG bond –––")
fprintf('1y fwds: %.2f IG; %.2f HY; %.2f Def \n',FV )
fprintf("Present Value in a year's time: %.2f\n",E_FV)
disp(' ')
...
for ii = 1 : N_issuers
    V = rho*Y+sqrt(1-R)*randn(N,1);           % AVR of i-th issuer
...
% Compute loss and VaR in default only case (DO)
profit=scen_D*L_D;
VaR_DO = -quantile(profit,0.001);

% Compute loss and VaR in default and migration case (DM)
profitm=scen_D*L_D+scen_d*L_d+scen_i*L_i;
VaR_DM = -quantile(profitm,0.001);
