function [price,IC] = NIG_MC_av(Nsim,sigma,volofvol,eta,ttm,F0,B,K)
...
mat_sim_av = repmat(sim_av, 1, l);
payoff_av = max(F0*exp(mat_sim_av) - mat_K,0);

% Compute Monte Carlo price and confidence interval
[price, ~, IC] = normfit(B*(payoff+payoff_av)/2);
