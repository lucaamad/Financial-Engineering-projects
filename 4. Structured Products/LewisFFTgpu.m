function [price,z]=LewisFFTgpu(B,F0,alpha,eta,sigma,volofvol,ttm,M,parameter,flag)

% Compute the price of a european call using the Lewis formula and
% approximating the integral with a fast Fourier transform, exploiting the
...
switch (flag)
    case 1
        if parameter<0
            x1=parameter;
        else % Check that x1 is negative, if it isn't we use the opposite
            x1=-parameter;
        end
...
% Convert output to workspace format
z=gather(z);
price=gather(price);

end % function LewisFFTgpu