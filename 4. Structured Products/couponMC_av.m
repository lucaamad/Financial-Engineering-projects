function [payoff, std_error] = couponMC_av(dividend, sigma, S0, weights, dates, discounts, mon_dates, P, Nsim)
...
% Simulate a 3-D random matrix (2,4,Nsim) which represents the Brownian
...
W_av = pagemtimes(W_av,diag(sqrt(dt)));

% Compute the value of the exponential of the GBM dynamics of the 
% stocks at each time and for each simulationSt = exp(drift+W);
St_av = exp(drift+W_av);
St=exp(drift+W);

% Compute S(t) 
St = pagemtimes(diag(weights),St);
S = sum(sum(St,1),2).*(1/Ndates);
St_av = pagemtimes(diag(weights),St_av);
S_av = sum(sum(St_av,1),2).*(1/Ndates);
...
% Compute the payoff
f1 = sum(max(S-P,0))/Nsim;
f2 = sum(max(S_av-P,0))/Nsim;
payoff = (f1+f2)/2;

% Standard error
std_error = std((max(S-P,0)+max(S_av-P,0))/2)/sqrt(Nsim);

end % function couponMC_av